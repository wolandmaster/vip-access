...
static void create_base_gui(app_context *context) {
  ...
  evas_object_smart_callback_add(context->window, "delete,request", delete_request_cb, NULL);
  ...
  context->main_context->secret = strdup(data_get(DATA_SECRET));
  if (validate_secret(context->main_context->secret)) {
    decode_secret((unsigned char *) context->main_context->secret);
  }
